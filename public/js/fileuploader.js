 ...
 */
const multer = require('multer'), mime = require('mime-types'), fs = require('fs'), gm = require('gm'),
    emptyFn = function () {
    }, error_messages = {
...
var Fileuploader = function (c, f, g, h) {
    return this.req = g, this.res = h, this.options = extendDefaults.call(this, f, {
        ...
        validate_file: null
    }), this.multer = multer({dest: this.options.uploadDir, fileFilter: fileFilter}), this.field = {
        name: c,
        input: [],
        listInput: null
    }, this
...
Fileuploader.prototype.getFileList = function (c) {
    ...
    var f = [], g = this.options.files, h = this.field.listInput;
    ...
    var c = [], f = Object.assign({}, this.options, {name: this.field.name});
    for (var g in f) {
        var h = f[g], j = 'data-fileuploader-' + g;
        ...
        }) : 'extensions' == g ? c.push({key: j, value: h.join(',')}) : 'name' == g ? c.push({
            key: g,
            value: h
        }) : 'required' == g ? c.push({key: g, value: ''}) : 'files' == g ? c.push({
            key: j,
    ...
    var f = this, g = 1 === f.options.limit ? 'single' : 'array', h = {
        hasWarnings: !1, isSuccess: !0, warnings: [], files: [], _callback: c, _setStatus: function (j, l, m) {
            var n = this, o = n._callback;
    ...
    f.multer[g](f.field.name)(f.req, f.res, function (j) {
        if (j) return h._setStatus(!1, {code: j.code, message: j.message}, !0);
        var l = 'single' == g ? [f.req.file] : f.req.files, m = f.req.body,
...
function handleUpload(c, f) {
    for (var g = this, h = g.options, j = g.field.listInput, l = 0; l < c.length; l++) {
        var m = {
            ...
            type: c[l].mimetype,
            size: c[l].size
        }, n = '0:/' + m.name, o = null === j || -1 < j.list.indexOf(n), p = pathinfo(m.name), q = new Date;
        ...
        if (!0 !== r) {
            if (!o) continue;
            f.files = [], f._setStatus(!1, {code: r, message: parseVariables(error_messages[r] || r, m)});
            break
        } else if (o) {
            var s = 0;
    ...
    return f.hasWarnings || f.files.forEach(function (t, u) {
        ...
        if (!isset(t.listProps)) {
            var w = j.list.indexOf(t.file);
            -1 < w && (t.listProps = j.values[w])
        }
...
function extendDefaults(c, f) {
    var g = this, h = Object.assign({}, f, c || {});
...
function pathinfo(c) {
    var f = c.substr(0, c.lastIndexOf('/')), g = -1 == c.indexOf('.') ? '' : c.split('.').pop().toLowerCase(),
        h = c.substr(f.length + (empty(f) ? 0 : 1), c.length - g.length - (empty(g) ? 0 : 1));
    return {path: f, title: h, extension: g}
}
...
function formatSize(c) {
    ...
    var f = 1e3, h = Math.floor(Math.log(c) / Math.log(f));
